  ...
  if (!pdfs || pdfs.length === 0) {
    return (
      <div className="h-full bg-white bg-opacity-70 backdrop-blur-md rounded-xl shadow-lg border border-gray-100 p-6 flex flex-col items-center justify-center text-center text-gray-500">
        ...
        <h2 className="text-xl font-semibold mb-2 text-gray-700">Visor de Documentos</h2>
    ...
    <div className="h-full bg-white bg-opacity-70 backdrop-blur-md rounded-xl shadow-lg border border-gray-100 flex flex-col overflow-hidden">
      ...
      <div className="flex overflow-x-auto border-b border-gray-200 bg-gray-50">
        {pdfs.map((pdf, index) => (
          <div
            key={index}
            className={`flex items-center gap-2 px-4 py-3 cursor-pointer border-r border-gray-200 min-w-max transition-colors ${
              activePdfIndex === index
                ? 'bg-white text-indigo-700 border-b-2 border-b-indigo-600 font-medium'
